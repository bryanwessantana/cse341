...
const createTrainer = async (req, res) => {
  try {
    ...
    if (response.acknowledged) {
      ...
      res.status(500).json('Error occurred while creating the trainer.');
    }
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
...
const updateTrainer = async (req, res) => {
  try {
    ...
    if (response.modifiedCount > 0) {
      ...
      res.status(500).json('Error occurred while updating the trainer.');
    }
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
...
const deleteTrainer = async (req, res) => {
  try {
    ...
    if (response.deletedCount > 0) {
      ...
      res.status(500).json('Error occurred while deleting the trainer.');
    }
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
